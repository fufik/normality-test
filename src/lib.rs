...
pub fn get_laplace_error(mut x:f32)-> f64 {
    let mut a = 1.0;
    if x<0.0{
        a = -1.0;
        x = -x;
    ...
    if x>=2.0{
        ...
        //println!("y laplace {}",y/2);
        return *LAPLACE_VAL.1.get((y/2) as usize).unwrap() * a
    }
    else{
        //println!("x laplace {}",(x*100.0).round());
...
pub mod dstrb_thr_freq{
    use std::ops::*;
    use crate::*;
    pub fn normal(gen_sample: &Vec<u32>,emp_freq: &Vec<u32>,mt: Vec<(u32,u32)>) -> Vec<f64>{
        //1.2Finding centres
        let mut m_cen : Vec<f32> = Vec::new();
        for i in mt.iter(){
            m_cen.push((i.0 as f32 + i.1 as f32)/2.0);
        }
        println!("Середины интервалов: {:#?}",m_cen);
        ...
        let mut mean_m_cen: f32 = 0.0;
        for (i,s) in m_cen.iter().enumerate(){
            mean_m_cen = mean_m_cen + ((m_cen[i] as f32)*(emp_freq[i] as f32));
        }
        println!("mean_m_cen: {:#?}", mean_m_cen);
        println!("emp_freq.len() {:#?}", emp_freq.len());
        //let mut a = 0;
        //for i in gen_sample.iter(){
        //    a = a + i;
        //}
        println!("Объём выборки: {:#?}",gen_sample.len());
        mean_m_cen = mean_m_cen/gen_sample.len() as f32;
        println!("Средняя выборочная {}",mean_m_cen);
        let mean_m2_cen = m_cen.iter()
                               .zip(emp_freq.iter())
                               .map(|f|((*f.0)*(*f.0)*(*f.1 as f32))as f32)
                               .sum::<f32>()
                               .div(gen_sample.len() as f32);
        println!("Средняя квадратов {}",mean_m2_cen);
        let st_dev = ((mean_m2_cen - mean_m_cen*mean_m_cen).mul(gen_sample.len() as f32).div((gen_sample.len()-1) as f32)).sqrt();
        //3 
        let mut a: Vec<f64> = Vec::new();
        for (i,_) in emp_freq.iter().enumerate(){
            
            if i==0 {
                a.push(get_laplace_error((mt[i].1 as f32 - mean_m_cen)/st_dev) - get_laplace_error(-10.0));
            } else if i==(emp_freq.len()-1){
        ...
        a = a.into_iter().map(|f| f*gen_sample.len() as f64).collect();
        return a;
    }
}
...
pub mod prove{       
    ...
    pub fn pearson_chi2_normal(mut gen_sample: Vec<u32>,alpha: f32,n_interv: usize) -> Result<bool,&'static str>{
        if (n_interv < 4||n_interv>gen_sample.len()){
            return Err("Too few intervals")
        }
        ...
        println!("efreqs: {:#?}",efreqs);
        println!("mt count {}",mt.len()); 
        //println!("efreqs {:#?}",efreqs); 
        //df=s-1-r
        //Normal Distribution has two degrees of freedom, hence r=2
        //s is amount of intervals
        let df: usize = efreqs.len()-1-2;
        //let df: usize = gen_sample.len() -1 -2;
        println!("Intervals: {:#?}",mt);
        let tfreqs = normal(&gen_sample,&efreqs,mt);
        println!("tfreqs: {:#?}",tfreqs);
        
        let chi2:f64 = efreqs.iter()
                         .zip(tfreqs.iter())
                         .map(|f| ((f.0*f.0) as f64)/f.1)
                         .sum::<f64>()
                         .sub(gen_sample.len() as f64);
        println!("chi2_empirical {:#?}",chi2);
        let tchi2 = get_chi2_crit(alpha,df).unwrap();
        println!("chi2_theoritical {}",tchi2);
        if chi2 < tchi2 as f64 {
            return Ok(true)
        }
        else{
            return Ok(false)
